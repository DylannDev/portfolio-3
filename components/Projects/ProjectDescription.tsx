  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6 mb-20">
        ...
        <h3 className="text-2xl sm:text-3xl font-bold">Contexte</h3>
        <div className="flex flex-col gap-6 text-lg">
          {selectedProject.paragraphs.map((paragraph, index) => (
            <p key={index}>{paragraph}</p>
          ))}
        </div>
        <h3 className="text-2xl sm:text-3xl font-bold">Features</h3>
          ...
          {selectedProject.features.map((feature, index) => (
            <li key={index} className="flex items-start sm:items-center gap-2">
              <div className="sm:pt-0">
                <PiCheckCircle className="text-purple text-xl" />
              </div>
              <p className="leading-5">{feature}</p>
            </li>
          ))}
        </ul>
        <div className="border-l-8 border-[#9d5cff]">
